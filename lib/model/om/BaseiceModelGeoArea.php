...
abstract class BaseiceModelGeoArea extends BaseObject  implements Persistent
{
  ...
  public function delete(PropelPDO $con = null)
  {
    ...
    try
    {
      $deleteQuery = iceModelGeoAreaQuery::create()
  ...
  protected function doInsert(PropelPDO $con)
  {
    ...
    if ($this->isColumnModified(iceModelGeoAreaPeer::ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ID`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::GEO_REGION_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`GEO_REGION_ID`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::GEO_CITY_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`GEO_CITY_ID`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::NAME_CYRILLIC))
    {
      $modifiedColumns[':p' . $index++]  = '`NAME_CYRILLIC`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::NAME_LATIN))
    {
      $modifiedColumns[':p' . $index++]  = '`NAME_LATIN`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::SLUG_CYRILLIC))
    {
      $modifiedColumns[':p' . $index++]  = '`SLUG_CYRILLIC`';
    }
    if ($this->isColumnModified(iceModelGeoAreaPeer::SLUG_LATIN))
    {
      $modifiedColumns[':p' . $index++]  = '`SLUG_LATIN`';
    }

    $sql = sprintf(
      'INSERT INTO `ice_geo_area` (%s) VALUES (%s)',
