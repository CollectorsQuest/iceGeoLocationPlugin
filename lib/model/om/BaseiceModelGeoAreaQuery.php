...
abstract class BaseiceModelGeoAreaQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelGeoAreaPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelGeoArea A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `GEO_REGION_ID`, `GEO_CITY_ID`, `NAME_CYRILLIC`, `NAME_LATIN`, `SLUG_CYRILLIC`, `SLUG_LATIN` FROM `ice_geo_area` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelGeoArea();
      $obj->hydrate($row);
      iceModelGeoAreaPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
