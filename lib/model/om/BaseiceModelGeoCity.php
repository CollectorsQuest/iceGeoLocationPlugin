...
abstract class BaseiceModelGeoCity extends BaseObject  implements Persistent
{
  ...
  public function delete(PropelPDO $con = null)
  {
    ...
    try
    {
      $deleteQuery = iceModelGeoCityQuery::create()
  ...
  protected function doInsert(PropelPDO $con)
  {
    ...
    if ($this->isColumnModified(iceModelGeoCityPeer::ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ID`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::GEO_REGION_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`GEO_REGION_ID`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::TYPE))
    {
      $modifiedColumns[':p' . $index++]  = '`TYPE`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::NAME_CYRILLIC))
    {
      $modifiedColumns[':p' . $index++]  = '`NAME_CYRILLIC`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::NAME_LATIN))
    {
      $modifiedColumns[':p' . $index++]  = '`NAME_LATIN`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::SLUG_CYRILLIC))
    {
      $modifiedColumns[':p' . $index++]  = '`SLUG_CYRILLIC`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::SLUG_LATIN))
    {
      $modifiedColumns[':p' . $index++]  = '`SLUG_LATIN`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::LATITUDE))
    {
      $modifiedColumns[':p' . $index++]  = '`LATITUDE`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::LONGITUDE))
    {
      $modifiedColumns[':p' . $index++]  = '`LONGITUDE`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::POSTAL_CODE))
    {
      $modifiedColumns[':p' . $index++]  = '`POSTAL_CODE`';
    }
    if ($this->isColumnModified(iceModelGeoCityPeer::TELEPHONE_CODE))
    {
      $modifiedColumns[':p' . $index++]  = '`TELEPHONE_CODE`';
    }

    $sql = sprintf(
      'INSERT INTO `ice_geo_city` (%s) VALUES (%s)',
      ...
      foreach ($modifiedColumns as $identifier => $columnName)
      {
        switch ($columnName)
        {
          ...
          case '`POSTAL_CODE`':
            ...
            break;
          case '`TELEPHONE_CODE`':
            $stmt->bindValue($identifier, $this->telephone_code, PDO::PARAM_STR);
            break;
        }
      }
      $stmt->execute();
