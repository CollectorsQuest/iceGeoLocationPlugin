...
abstract class BaseiceModelGeoCityQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelGeoCityPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelGeoCity A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `GEO_REGION_ID`, `TYPE`, `NAME_CYRILLIC`, `NAME_LATIN`, `SLUG_CYRILLIC`, `SLUG_LATIN`, `LATITUDE`, `LONGITUDE`, `POSTAL_CODE`, `TELEPHONE_CODE` FROM `ice_geo_city` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelGeoCity();
      $obj->hydrate($row);
      iceModelGeoCityPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
