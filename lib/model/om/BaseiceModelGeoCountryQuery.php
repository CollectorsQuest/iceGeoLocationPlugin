...
abstract class BaseiceModelGeoCountryQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelGeoCountryPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelGeoCountry A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `NAME`, `SLUG`, `ISO3166`, `CURRENCY`, `LATITUDE`, `LONGITUDE`, `ZOOM` FROM `ice_geo_country` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelGeoCountry();
      $obj->hydrate($row);
      iceModelGeoCountryPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
