...
abstract class BaseiceModelGeoStreetQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelGeoStreetPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelGeoStreet A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `GEO_REGION_ID`, `GEO_CITY_ID`, `GEO_AREA_ID`, `NAME_CYRILLIC`, `NAME_LATIN`, `SLUG_CYRILLIC`, `SLUG_LATIN` FROM `ice_geo_street` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelGeoStreet();
      $obj->hydrate($row);
      iceModelGeoStreetPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
