 ...
 */
class PluginiceModelGeoCity extends BaseiceModelGeoCity
{
  /**
  ...
  {
    if ($this->getNameCyrillic() == $this->geticeModelGeoRegion()->getNameCyrillic())
    {
      $name = $this->getFullName('bg_BG');
    }
    else
    {
      $name = $this->getFullName('bg_BG') .' '. $this->iceModelGeoRegion()->getFullName('bg_BG');
    }
    $this->setSlugCyrillic(Utf8::slugify($name, '-', false, false, 'n-a'));

    if ($this->getNameLatin() == $this->geticeModelGeoRegion()->getNameLatin())
    {
      $name = $this->getFullName('en_US');
    }
    else
    {
      $name = $this->getFullName('en_US') .' '. $this->geticeModelGeoRegion()->getFullName('en_US');
    }
